    ...
    return `
        <div class="Product">
            <div class="Product_hover">
                ...
                <div id="action_btn">
                    <span class="icon_btn"><img src="${product.shareIcon}" alt=""> <p>Share</p></span>
                    <span class="icon_btn"><img src="${product.compareIcon}" alt=""> <p>Compare</p></span>
                    <span class="icon_btn"><img src="${product.likeIcon}" alt=""> <p>Like</p></span>
                </div>
            </div>
            <div class="img_label">
                <img class="Product_img_1" src="${product.image}" alt="${product.name}">
                <span class="label"><img src="${product.label}" alt="Label"></span>
            </div>
            <div class="Product_content">
                ...
                <span class="Prices">
                    ...
                    ${product.previousPrice ? `<p class="Previous_Price">${product.previousPrice}</p>` : ''}
                </span>
            </div>
        </div>
    `;
...
function renderProductList(products) {
    ...
    productList.innerHTML = limitedProducts.map(product => generateProductHTML(product)).join('');
}
...
function renderPaginationControls(totalPages) {
    ...
    // Previous button
    buttons += `<button id="Previous" onclick="changePage(${currentPage - 1})" ${currentPage === 1 ? 'class="disabled"' : ''}>Previous</button>`;

    // Page numbers
    for (let i = 1; i <= totalPages; i++) {
        buttons += `<button id="number" onclick="changePage(${i})" class="${currentPage === i ? 'active' : ''}">${i}</button>`;
    }

    // Next button
    buttons += `<button id="Next" onclick="changePage(${currentPage + 1})" ${currentPage === totalPages ? 'class="disabled"' : ''}>Next</button>`;
